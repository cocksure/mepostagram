...
class SignUpSerializer(serializers.ModelSerializer):
    ...
    def auth_validate(data):
        ...
        if input_type == 'email':
            ...
            error_message = 'Telefon yoki email kiriting!'
    ...
    def validate_email_phone_number(self, value):
        ...
        if value and User.objects.filter(email=value).exists():
            data = {
                "success": False,
                "message": "Bu email oldin ishlatib bolingan"
            }
            raise ValidationError(data)
        elif value and User.objects.filter(phone_number=value).exists():
            data = {
                "success": False,
                "message": "Bu telefon raqam oldin ishlatib bolingan"
            }
            raise ValidationError(data)
...
class ChangeUserInformation(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if password != confirm_password:
            raise ValidationError(
                {'message': 'Paroliz bir biriga togri kemayapti!'}
            )
        if password:
    ...
    def validate_username(self, username):
        if len(username) < 5 or len(username) > 30:
            raise ValidationError(
                {
                    "message": "Username must be beetwen 5 and 30 characters"
                }
            )
        if username.isdigit():
            raise ValidationError(
...
class LoginSerializer(TokenObtainSerializer):
    ...
    def auth_validate(self, data):
        ...
        if check_user_type(user_input) == 'username':
            ...
            user = self.get_user(email__iexact=user_input) #get orqali user ozgaruvchiga biriktirildi
