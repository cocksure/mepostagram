...
class GetNewVerification(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return Response(
            {
                "success": True,
                "message": "Tasdiqlash kodingiz qayta jonatildi"
            }
        )

    @staticmethod
    def check_verication(user):
        verifies = user.verify_codes.filter(expiration_time__gte=datetime.now(), is_confirmed=False)
        if verifies.exists():
            data = {
                "message": "Kodingiz xali yaroqli, Biroz kuting"
            }
            raise ValidationError(data)
...
class ChangeUserPhotoView(APIView):
    ...
    def put(self, requset, *args, **kwargs):
        serializer = ChangeUserPhotoSerializer(data=requset.data)
        if serializer.is_valid():
            user = requset.user
            serializer.update(user, serializer.validated_data)
            return Response(
                {"message": "Rasm muvaffaqiyatli ozgardi"}, status=200)
        return  Response(
...
class ResestPasswordView(UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        response = super(ResestPasswordView, self).update(request, *args, **kwargs)
        try:
        ...
        return Response(
            {
                'success': 'Parolingiz muvaffaqiyatli ozgardi!',
                'access': user.token()['access'],
                'refresh': user.token()['refresh_token'],
